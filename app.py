...
class ConfigManager:
    ...
    def get_default_config(self) -> dict:
        ...
        return {
            'video': {
                'input_source': 'vecteezy_ho-chi-minh-city-traffic-at-intersection-vietnam_1793410.mov',
                'frame_skip': 2,
                'resize_width': 1280,
                'resize_height': 720
            },
...
class IndustrialTrafficMonitor:
    ...
    def draw_clean_overlay(self, frame, metrics, kpi_metrics, predicted_flow, fps):
        ...
        summary_lines = [
            ...
            f"Total Vehicles: {sum(self.class_counts.values())}",
            f"Cars: {self.class_counts.get('car', 0)}",
            f"Bikes: {self.class_counts.get('motorcycle', 0)}",
            f"Buses: {self.class_counts.get('bus', 0)}",
            f"Trucks: {self.class_counts.get('truck', 0)}"
        ]
